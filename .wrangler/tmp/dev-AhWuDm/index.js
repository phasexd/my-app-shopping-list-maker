"use strict";
(() => {
  // .wrangler/tmp/bundle-QfZTRM/checked-fetch.js
...
`
  ...
  // ../../.bun/install/global/node_modules/wrangler/templates/middleware/loader-sw.ts
  ...
  var __Facade_ExtendableEvent__ = class extends Event {
    ...
    waitUntil(promise2) {
      ...
      this[__facade_waitUntil__].push(promise2);
    }
  };
  ...
  // .wrangler/tmp/bundle-QfZTRM/middleware-insertion-facade.js
  ...
  var createPostSchema = itemSchema.omit({ id: true });
  var FakeItems = [
    {
      id: 1,
      title: "Pen",
  ...
  var itemsRoutes = new Hono2().get("/", (c) => {
    ...
    const item = FakeItems.find((item2) => item2.id === id);
    if (!item) {
      return c.notFound();
    }
    return c.json({ item });
  ...
  app.use("*", logger());
  app.use("/api/*", cors());
  var apiRoutes = app.basePath("/api").route("/items", itemsRoutes);
